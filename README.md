...
/init
```
Lalu `roadmap` (opsional — susun backlog fitur), `architect` (fondasi teknis), `wire` (bring-up skeleton kosong-tapi-jalan), `feature` (bikin fitur), `ship`/`drop` (lifecycle), `render-docs` (doc human-readable).
...
```
Buat ide yang masih mentah. **Q&A visi operator dulu, baru riset** (elicit-first); asesmen compliance cuma opt-in bila produk kena sinyal regulated (uang/PII berat/sektor). Nol teknis. Output: dok strategis HTML (`control/docs/discovery.html`) + seed awal `control/business/`; di akhir otomatis panggil `/init` + tawarkan `/roadmap`. Tiap klaim disitasi + dilabeli keyakinan; `critic` menantang sebelum kamu menerima.
...
Urutan greenfield (ide jelas): `/init` -> `/roadmap` (opsional) -> `/architect` -> `/wire` -> `/feature` -> `/breakdown` -> `/build` -> `/ship`.
Urutan greenfield (ide mentah): `/discovery` -> `/init` -> `/roadmap` (opsional) -> `/architect` -> `/wire` -> `/feature` -> `/breakdown` -> `/build` -> `/ship`.
...
> `breakdown` kini bisa wakili kerja non-file (`actions:` migrate/install/env) & langkah manusia (`manual:`/status `needs_human`); `build` jalanin+verifikasi actions (migrasi lewat gate; unattended: additive auto-apply ber-opt-in, destructive nunggu lo) dan uji integrasi cross-app; `build`/`ship` sadar multi-repo (branch & PR per repo).
