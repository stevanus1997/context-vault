...
1. **Langkah 1 = SELESAI & LIVE di `main`** (@ `54632e4`). **H2 (gap pertama Langkah 2) = SELESAI & LIVE di `main`** (@ `fa62982`). Jangan ulang keduanya.
2. **Langkah 2 BELUM kelar** — sisa **M5 → M4 → H3** (urutan dari audit; M4 mengaktifkan H3). **Mulai dari M5.** Tiap gap = **spec terpisah** (subsistem agak independen, jangan dijejal 1 spec).
...
```
**16 skill** + 2 agent (`critic`, `security-critic`). Desain induk: `docs/superpowers/specs/2026-05-24-ai-first-boilerplate-design.md`.
...
## 3. LANGKAH 2 — SISA (garap berurutan: M5 → M4 → H3)

> H2 sudah DONE (§2.2). Mulai dari **M5**. Tiap gap = spec+plan sendiri. Usulan fix di bawah = titik-awal brainstorming, BUKAN keputusan final — selalu lewati `brainstorming` + AskUserQuestion dulu.

### M5 — Integrasi vendor tak punya rumah durable (severity: MEDIUM) ⭐ MULAI DARI SINI
...
**State git saat handoff:** `main` == `origin/main` @ `fa62982` (Langkah 1 + H2 live), working tree clean, tak ada branch nyangkut.
