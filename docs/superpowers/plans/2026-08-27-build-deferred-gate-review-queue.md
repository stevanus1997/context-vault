...
|---|---|---|
| `~/Developer/project/branding/control/features/login` | `risk: high`, 25 task, 7 milestone | 0 stop; semua segmen `queued` (risk:high) kecuali tidak ada `auto`; `security-critic` jalan di segmen ber-verba (M1–M4, FIX, FIX2); `outcome: review`, `review: 7`, `blockers: 0`; blast-radius G1 (T1) = T4, T5, T9, T10, T13, T15 (cek `files:` tumpang-tindih) |
| `~/Developer/project/branding/control/features/generate-website` | T7 `migrate` `kind: additive` (3 tabel + RLS), `risk: high` | apply di-allowlist → T7 cross-check DDL lolos (`CREATE`/`ENABLE ROW LEVEL SECURITY` bukan verba destruktif) → apply → `done` → gate `queued` "ddl additive T7 (auto-applied)"; apply TIDAK di-allowlist → T7 `needs_human` + `hold: "allowlist migrate absen — wire 5.5"`, dependents T7 nunggu, task lain lanjut |
