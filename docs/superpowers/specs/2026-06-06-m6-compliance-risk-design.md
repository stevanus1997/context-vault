...
**Tujuan**
- File durable `control/business/risks.md` (cermin gaya `invariants.md`): slot saran **PCI · GDPR/Privasi · Pajak · KYC/AML** + baris bebas; tiap slot **isi ATAU `N/A — alasan`**; sentinel `<belum dinilai>`; bentuk entri *area · pemicu · kewajiban · label keyakinan · sumber*. Auto-scaffold `init` (sudah copy `business/*.md` + ganti `<PRODUCT>`).
