...
- `migrate` unattended, baca `kind`:
  - **`additive`** → (1) **cross-check deterministik:** grep file migrasi di diff task untuk verba destruktif (`DROP`, `RENAME`, `ALTER … TYPE`, `SET NOT NULL` tanpa default, `TRUNCATE`, `DELETE FROM`, `UPDATE … SET`) — kena = `kind` bohong → perlakukan destructive; (2) perintah apply di-allowlist? tidak → hold `needs_human` + `hold: "allowlist migrate absen — wire 5.5"`; (3) jalankan `rules/migration-impact.md` → `gate-Gn-impact.md`; (4) apply + verifikasi + regen `control/schema/<unit>.md` (M4) → `done`; gate segmen nanti `queued` ("ddl additive Tn (auto-applied)").
  ...
  - Attended: tidak berubah (tampilkan rencana + dampak + approve).
...
8. Mode attended **byte-identik** perilakunya di luar drain.
...
4. **Desk-check skenario nyata** (`tasks.yaml` asli; ekspektasi ditulis di plan): `login` (`risk:high`) → 0 stop, `review: 6`, `auto: 1`, blast-radius G1 = T4/T5/T9/T10/T13/T15 · `generate-website` T7 additive → di-allowlist: apply + `queued` "ddl additive"; tidak: `needs_human` + dependents nunggu, 33 lain lanjut · `simplify-scoring-stp` → `queued: 1`, bukan stop · `marketing-landing` → `done` seperti dulu (**regresi nol**) · hipotetis: destructive → `needs_human` + subtree; `NEEDS_CONTEXT` → `needs_human`; 2 `blocked` sistemik → `halt`; drain revisi G1 → corrective milestone sama → due lagi.
