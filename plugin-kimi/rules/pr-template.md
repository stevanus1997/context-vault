...
2. **Ada template → pakai PERSIS sebagai body:**
   - **Dir multi-file:** pilih file yang cocok `rasa` by nama (mis. `fix.md`/`feature.md`/`bug.md`); tak ada yang cocok → **TANYA user** pilih mana (ikuti `${KIMI_SKILL_DIR}/../../rules/elicitation.md` — opsi bawa konsekuensi, sediakan jalan keluar; jangan tebak).
