...
### 2. Petakan ke app
Cocokkan kebutuhan fitur ke `capabilities`/`responsibility` tiap app → tentukan app yang kena + apa perannya. **Q&A/konfirmasi ke user ikuti `${KIMI_SKILL_DIR}/../../rules/elicitation.md`** — tiap keputusan-bercabang di bawah (app/package/vendor/design-system baru) ditanya satu per giliran dengan konsekuensinya, jangan diborong.
