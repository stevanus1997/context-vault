...
**Verba-uang (PLUMBING):** charge/capture · refund · payout/settlement/transfer · simpan PAN/instrumen-bayar/token-kartu. (Heuristik sensitivity `intake` + `${KIMI_SKILL_DIR}/../../rules/compliance-risk.md` = PENGUAT, bukan classifier utama — daftar verba ini yang operasional.)
...
- **Body+judul:** ikut `${KIMI_SKILL_DIR}/../../rules/pr-template.md` (`rasa` = `tweak`). Bentuk fallback tweak: `Summary · Rationale · Changes · Challenge Checklist · Testing · Capture` — TANPA section Flow & Traceability (atomik). Judul `tweak(<unit>): …`.
