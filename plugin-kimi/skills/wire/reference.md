...
**Beda dengan build:** wire = **SEKALI, fondasi** (skeleton kosong-tapi-jalan); build = **PER FITUR** (kode fitur ke skeleton). wire bikin **pipeline migrasi BERFUNGSI + baseline** (kosong table fitur); build bikin **TABLE fitur**. Dua-duanya gate `migrate`. **Proyeksi skema (M4):** wire juga **melahirkan `control/schema/<app>.md`** (proyeksi, `label=<none>`) per `${KIMI_SKILL_DIR}/../../rules/schema-projection.md`; build me-regen-nya tiap task-migrate app `done`.
