...
Baca `<work-item>/tasks.yaml` (work-item = `control/features/<fitur>/` ATAU `control/fixes/<id>/`) + (fitur) `plans/*` + `_shared.md` + `control/conventions.md` + `control/workspace.yaml` (path/stack). **Prasyarat:** `tasks.yaml` ada (kalau belum → suruh jalankan `breakdown` dulu, sebaiknya sesi terpisah). **Manifest work-item HARUS aktif:** `feature.yaml` `status: active` (work-item `features/<fitur>/`) **ATAU** `fix.yaml` `status: open`/`diagnosed` (work-item `fixes/<id>/`, lane bugfix). Kalau manifest **closed** (`shipped`/`dropped`) atau fitur `draft`, BERHENTI & jelaskan. Untuk work-item fix, `plans/*` boleh tak ada (kontrak ringan); `_shared.md` mini **wajib** bila fix lintas-unit.
