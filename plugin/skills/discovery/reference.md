...
- **Risiko** — Apa yang bisa bikin gagal (pasar jenuh, switching cost tinggi, regulasi, beratnya eksekusi)? **Compliance (durable, carve-out ke `risks.md` — CONDITIONAL opt-in, SKILL.md step 4; heuristik pemicu uang/PII-berat/sektor-regulated + SATU pertanyaan; di-skip → `risks.md` tetap skeleton, degrade M6):** nilai terstruktur kewajiban regulasi — **PCI** (kartu/bayar) · **GDPR/privasi** (data pribadi) · **pajak** (jurisdiksi/PPN) · **KYC/AML** (identitas) + regulasi sektor/jurisdiksi spesifik. Lihat `${CLAUDE_PLUGIN_ROOT}/rules/compliance-risk.md`.
